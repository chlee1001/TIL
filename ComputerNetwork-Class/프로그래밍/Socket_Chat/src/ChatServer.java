	...
	public static void main(String[] args) throws Exception {
		...
		try {
			while (true) {
				...
				SocketAddress clientHost = listener.getLocalSocketAddress();
	...
	private static class Handler extends Thread {
		...
		public void run() {
			try {
				...
				/**
				 * Created by chlee on 2017-10-06.
				 */
				
				
				// if new user connected then send the message to everyone
				for (PrintWriter writer : writers) { 
					writer.println("MESSAGE " + "User " + name + " Entered");
					writer.flush();
				}
				...
				while (true) {
					...
					String command[] = input.split("/w "); // To extract the whisper command.
					// Object[] nameArray = names.toArray();
					// Object[] writerArray = writers.toArray();

					if (input == null) {
						return;
					} 
					
					// if client send "!online' then server send the list of users
					else if (input.equalsIgnoreCase("!online")) {
						PrintWriter users = new PrintWriter(socket.getOutputStream(), true);
						users.println("MESSAGE " + "Online users : \n");
						for (String current_user : names) {
							users.println("MESSAGE " + current_user + "\n");
						}

					}

					// if client send with whisper command
					else if (command.length > 1) { // if exist the command
						String receiver[] = command[1].split(" ");
						for (int i = 0; i < writers.size(); i++) {
							 // find the receiver and sender then server send the message to receiver and sender
							if ((receiver[0].equals(names.get(i)) || names.get(i).equals(name))) {
								writers.get(i).println("MESSAGE " + name + ": " + command[1].substring(names.size()));
								// ((PrintWriter) writerArray[i]).println("MESSAGE " + name + ": " + input);
								// ((PrintWriter) writerArray[i]).flush();
							}
						}
					}
				...
				if (name != null) {
					for (PrintWriter writer : writers) {
						writer.println("MESSAGE " + "User " + name + " Exited");
					}
					names.remove(name);
				}
				if (out != null) {
